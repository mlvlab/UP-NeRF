...
@torch.no_grad()
def tb_image(
    opt, tb, step, group, name, images, num_vis=None, from_range=(0, 1), cmap="gray"
):
...
def plot_save_poses_blender(opt, fig, pose, pose_ref=None, path=None, ep=None):
    ...
    ax.add_collection3d(
        Poly3DCollection([v[:4] for v in cam_ref], alpha=0.2, facecolor=ref_color)
    )
    for i in range(N):
        ax.plot(
            cam_ref[i, :, 0],
        ...
        ax.scatter(
            cam_ref[i, 5, 0], cam_ref[i, 5, 1], cam_ref[i, 5, 2], color=ref_color, s=20
        )
    if ep == 0:
...
def get_pose_image(
    pose, pose_ref, cam_depth=0.1, elev=-90, azim=-90, x=(-1, 1), y=(-1, 1), z=(-1, 1)
):
    # get the camera meshes
    ...
    ax1 = fig.add_subplot(121, projection="3d")
    ax2 = fig.add_subplot(122, projection="3d")
    setup_3D_plot(ax1, elev=elev, azim=azim, lim=edict(x=x, y=y, z=z))
    setup_3D_plot(ax2, elev=0, azim=-90, lim=edict(x=x, y=y, z=z))
    ax1.set_title("forward-facing view", pad=0)
    ax2.set_title("top-down view", pad=0)
    plt.subplots_adjust(left=0, right=1, bottom=0, top=0.95, wspace=0, hspace=0)
    plt.margins(tight=True, x=0, y=0)
